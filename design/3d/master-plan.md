...
| **M6** | ◐ graphics done | **Graphics overhaul (WS1–WS10) shipped:** quality tiers (high/medium/low + ?tier/?tod/?weather/?bloodmoon dev params), §3.9 post-FX stack (HDR pipeline: keyframed bloom/exposure/contrast/vignette/ColorCurves + per-biome grade nudges, grain/chromatic/sharpen, SSAO2, hurt pulse, storm flash, death pull, encounter DOF beat + dolly), sky dome (gradient stops, stars, blood-moon/volcanic ember), §3.8 CSM shadows (2 cascades, streamed renderList) + Low-tier blob discs, ground/wall vertex richness (per-tile jitter, corner AO, biome tints, wall base gradient + top trim, façade windows, door frames), §3.5.2 visual terrain relief (raise-only 0.4m through groundHeightAt; flat under structures/water; sim-invisible, tested), humanoid v2 rigs (legs, shoulder pivots, eye strips, LookSpec feature add-ons, hit-flash, death poses replace slabs), prop facelift (wheeled glassed vehicles, stocked shelving…), FX texture port (splat decals in blood colours, glows/beams, soft muzzle/explosions), warm light pool ≤8 (lava + night doorways, flicker, crossfade), atmosphere pools (fireflies/leaves/dawn mist/embers). 4 new pure test suites; smoke gate runs tier=low + a medium errors-only boot. Still pending from M6: PannerNode positional audio, GLB manifest/AssetRegistry lane, title diorama, DOM modal restyle. |
| **M6.5** | ✅ animation | **§3.7 animation system (anim WS1–WS8) shipped, 100% procedural:** pure pose-composition core (`render3d/anim/`: easing kit, `AnimController` — locomotion/one-shot/stance/additive layers, priority death 100 > spawn 90 > hit 80 > attack 60 > recoil 40, sim-time clock so hitstop/pause freeze poses); rig v3 (torso/neck pivots, two-segment arms+legs, hand sockets; quad leg/neck/tail pivots) behind stable `applyPose`/`applyQuadPose` writers; full locomotion cycles (counter shoulder/hip swing, swing-leg knees, footfall bob + plant-moment dust, archetype layers — crawler heave, lurcher gather, stalker creep, bloated `GAITS.shelled` waddle, erratic twitch, runner pump; parity freqs stay the phase source, amplitude scales with `speedFrac` — no foot-slide); player combat (weapon-in-hand blockouts total over all 21 WeaponClasses, slash/thrust/smash one-shots on the swing event, the §3.7 fire-recoil window, aim stance tracking `aimDeltaYaw`, reload/rummage loops via polled flags); enemy reactions (lunge on `enemyAttack`, directional stagger off splat, spit/scream synced to their rings, leap/tumble stances); deaths & spawns (flop/crumple/launch tweens whose t=1 equals the `poseCorpse` contract — test-asserted `DEATH_END` — before the SAME rig hands to the corpses map; spawn claw-up; animal pop-in/tip-over); real quadruped gaits (rabbit bound w/ parabolic hop, deer gallop + boar trot riding the mandated `engine/anim.ts` `gaitPose`/GAITS table, flee stride ×1.35, the 2D yaw parity verbatim); world transients (pooled chest lids −110° + gold sparks + 700ms light blip on real-chest `searchDone`, drop bounce-arc → bob handoff, magnet flight on the sim clock, camera walk-bob + ranged aim-side lead through FollowRig). Sim edits ADDITIVE only: `enemyAttack`/`enemySpit` events, `screamRing.id`, `enemyRemoved` death context, `EnemySim.isLeaping`. Rig LOD: half-rate tick beyond 900px. ~90 new pure checks in `tests/anim3d.test.ts` + sim hooks in simcore; `scripts/animProbe.mjs` (live joint swings, forced-kill death contract, quad gait, drop arc/magnet/lid) and `scripts/animShots.mjs` (the 4 plan screenshot recipes) verify the running build under SwiftShader. |
